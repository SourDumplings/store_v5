						...
						<table cellspacing="0" cellpadding="1" rules="all" bordercolor="gray" border="1" id="DataGrid1"
							...
							<c:forEach items="${allOrders }" var="o" varStatus="status">
								<tr onmouseover="this.style.backgroundColor = 'white'" onmouseout="this.style.backgroundColor = '#F5FAFE';">
									...
									<td style="CURSOR: hand; HEIGHT: 22px" align="center" width="5%">${o.name }</td>
									<td style="CURSOR: hand; HEIGHT: 22px" align="center" width="5%"><c:if test="${o.state == 1}">未付款</c:if> <c:if test="${o.state == 2}">
											<a href="/store_v5/AdminOrderServlet?method=updateOrderByOid&oid=${o.oid }">发货</a>
										</c:if> <c:if test="${o.state == 3}">已发货</c:if> <c:if test="${o.state == 4}">订单完成</c:if></td>
									<td align="center" style="HEIGHT: 22px" width="60%"><input type="button" value="订单详情" class="myClass" id="${o.oid }" />
										<table border="1" width="100%">
											...
											<tr>
												<td><img alt="" src="/store_v5/products/1/c_0001.jpg"
													width="50px"></td>
												<td>XXX</td>
												<td>XXX</td>
												<td>XXX</td>
...
<script type="text/javascript">
	$(function()
	// 页面加载完毕后我们获取样式名称为myClass的一类元素，为期绑定点击事件
	{
		// alert("1");
		$(".myClass").click(
				function()
				{
					...
					if (txt == "订单详情")
					{
						...
						$.post(url, obj, function(data)
						{
							...
							{
								var td = "<tr><td><img src='/store_v5/"+obj.product.pimage+"' width='50px'></td><td>" + obj.product.pname + "</td><td>" + obj.product.shop_price + "</td><td>"
										+ obj.quantity + "</td></tr>";
								$tb.append(td);
							});
						}, "json");
